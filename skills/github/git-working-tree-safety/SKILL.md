...
3. Verify the exact remote and branch you intend to push to.
   ...
   ```
   Use the count to decide whether a push is needed (`N\t0`), unnecessary (`0\t0`), or should be preceded by a rebase/merge (`0\tN`). If the local checkout contains the desired newer tree but the named publication branch has diverged, merge the target branch into the local branch with `git merge --no-ff <remote>/<branch>` before staging the intended changes; this preserves the target branch history and makes the subsequent explicit push fast-forwardable. Conclude the merge only after reviewing the staged set, and never force-push merely to overwrite a diverged publication branch.
...
- In noisy session-root checkouts like `/root/.hermes`, the real code checkout may be the parent root itself or a nested repo below it; verify the owning repo's branch, remotes, and `HEAD` before staging or pushing anything.
  ...
  - For recurring `/root/.hermes` publish jobs, inspect the outer root and every nested repo separately before deciding there is nothing to commit. A nested repo can be clean while the outer root still has publishable additions; in that case, confirm the durable files with `git ls-tree HEAD` or `git diff --name-only` and stage only the intended allowlist paths (for example `skills/.archive/...`) instead of broad-root staging.
  ...
  - If the outer root shows an untracked binary or build artifact (for example under `bin/`), verify whether it is a tracked project asset, nested-repo output, or live runtime state before staging; do not assume it belongs in the publish target just because it lives under `/root/.hermes`.
  - If multiple nested repos are relevant to the same cron job, check each repository and branch explicitly before returning `[SILENT]`; a clean child repo does not imply the outer root is clean.
  - When the outer checkout and a nested checkout use the same GitHub repository URL, treat them as separate Git histories that may contend for the same remote branch. Do not push the parent gitlink commit to a branch just published from the nested repository; first determine which repository owns the destination ref. Publish the intended repository once, and leave a parent-only pointer update local unless the parent has its own distinct remote branch/ref.
  - If a push reports `fetch first` after another checkout published the same branch, fetch the authoritative remote ref, compare ancestry, and rebase the intended commit onto that tip before retrying. Never force-push merely to reconcile two checkout layers.
