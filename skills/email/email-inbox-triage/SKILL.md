...
### 7. Scheduled monitoring mode

For recurring inbox/calendar checks, load persistent deduplication state before retrieval and compare every candidate by provider ID **and** a stable normalized hash. Normalize legacy hash prefixes before comparison. Apply relevance and hard-noise filters first; then remove all previously notified candidates before rendering output. If nothing remains, return the exact configured silence token. If candidates remain, persist the exact final alert list and its IDs/hashes atomically in the same run before returning it. Treat provider reminder emails as noise when the underlying calendar item is already covered by the calendar scan. When a search snippet is generic or HTML-heavy, fetch the full message before committing an alert.
