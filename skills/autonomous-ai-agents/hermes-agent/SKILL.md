...
# 1. Stash local changes (if any), pull latest, restore
git stash --include-untracked && git pull origin main && git stash pop

# 2. Reinstall package + deps (uses uv, not pip)
...
# or: cat ~/.hermes/.update_check  # JSON with last check timestamp and commits-behind count
```
