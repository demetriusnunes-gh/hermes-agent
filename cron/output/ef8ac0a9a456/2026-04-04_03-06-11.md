...
### Primary: mcporter native call

This is the working method. Use `--http-url` triggers OAuth flow and times out — avoid it.

```bash
npx mcporter call zapier.gmail_find_email instructions="find recent emails in my inbox" output_hint="show sender name, sender email, subject, date, and first 100 characters of the body" query="in:inbox newer_than:2h" --output json 2>&1
```

**IMPORTANT**: Do NOT use `npx mcporter call --http-url "$ZAPIER_URL"` — this triggers an OAuth browser flow that times out after 60s in headless/cron contexts. Always use the native `zapier.gmail_find_email` form.
...
## Fallback: Gmail IMAP (when Zapier MCP is unreachable)

Both `~/.hermes/config.yaml` and `/root/config/mcporter.json` have masked/truncated Zapier tokens (`***` and `ZWZhND...c9` respectively). The native mcporter call always fails, and `--http-url` requires interactive terminal approval (won't work in cron/headless).

Use Gmail IMAP directly as a reliable fallback:
...
app_pw = open(os.path.expanduser('~/.gmail-app-password')).read().strip()
mail = imaplib.IMAP4_SSL('imap.gmail.com')
mail.login('demetriusnunes@gmail.com', app_pw)
mail.select('INBOX')

# Search recent emails (IMAP SINCE is date-only, filter by time after)
since_date = (datetime.now(timezone.utc) - timedelta(hours=24)).strftime('%d-%b-%Y')
status, messages = mail.search(None, f'(SINCE "{since_date}")')

two_hours_ago = datetime.now(timezone.utc) - timedelta(hours=2)
for msg_id in messages[0].split():
    status, data = mail.fetch(msg_id, '(BODY.PEEK[HEADER.FIELDS (FROM SUBJECT DATE MESSAGE-ID)])')
    # Parse headers, check time window, check relevance criteria
    # ... (see /tmp/check_emails_v2.py for full implementation)
```
...
If both IMAP and MCP fail, report: "⚠️ Email check skipped — Gmail unreachable."
...
1. Nubank <todomundo@nubank.com.br>
   ...
   Fri, 03 Apr 2026 05:12:21 +0000 — --e8f50b002687d333e514ca44afdec33be2cb4166d727cbcfe711a1916443\r Content-Type: text/html; charset="utf-8"\r Content-Transfer-Encoding: quoted-printable\r \r <!doctype html><html xmlns=3D"http://www.w3.org
   → Financial/purchase related (bank)

2. "Amazon.com.br" <order-update@amazon.com.br>
   Subject: Seu pedido Amazon Now foi recebido
   Fri, 3 Apr 2026 13:05:16 +0000 — ------=_Part_1115132_196751301.1775221516511\r Content-Type: text/plain; charset=utf-8\r Content-Transfer-Encoding: quoted-printable\r \r \r  =20\r \r Amazon Now\r \r https://www.amazon.com.br/alm/storefront?a
   → Financial/purchase related (order)

3. "Amazon.com.br" <shipment-tracking@amazon.com.br>
   Subject: Enviado: "BARRA PROT PINC CHOCOLATE..."
   Fri, 3 Apr 2026 21:06:57 +0000 — ------=_Part_8916_334364631.1775250417760\r Content-Type: text/plain; charset=utf-8\r Content-Transfer-Encoding: quoted-printable\r \r \r  =20\r \r \r     Seu pacote foi enviado!\r Pedido feito\r \r Enviado\r \r S
   → Financial/purchase related (amazon)

4. "Amazon.com.br" <shipment-tracking@amazon.com.br>
   Subject: Enviado: "Contos indígenas brasileiros"
   Fri, 3 Apr 2026 21:09:31 +0000 — ------=_Part_16746_863571904.1775250571046\r Content-Type: text/plain; charset=utf-8\r Content-Transfer-Encoding: quoted-printable\r \r \r  =20\r \r \r     Seu pacote foi enviado!\r Pedido feito\r \r Enviado
   → Government-related (IR)

5. "Amazon.com.br" <shipment-tracking@amazon.com.br>
   Subject: Enviado: 1 produto. Seu pedido está a caminho!
   Sat, 4 Apr 2026 02:36:06 +0000 — ------=_Part_194915_519108357.1775270166266\r Content-Type: text/plain; charset=utf-8\r Content-Transfer-Encoding: quoted-printable\r \r \r  =20\r \r \r     Seu pacote foi enviado!\r Pedido feito\r \r Enviado
   → Financial/purchase related (amazon)

📅 Upcoming events:
No upcoming events found for today and tomorrow.

💾 Updated state with 5 new notifications to prevent duplicate reporting.
