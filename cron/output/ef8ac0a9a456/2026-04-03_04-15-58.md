...
⚠️ **Email check failed — Zapier MCP token expired.**

The OAuth token for Zapier MCP authentication is invalid. Both `npx mcporter` calls and direct HTTP requests to the endpoint returned:
...
**To fix this:**
1. Go to your Zapier MCP dashboard
2. Re-generate the OAuth token
3. Update `mcp_servers.zapier.url` in `~/.hermes/config.yaml`

The fallback himalaya CLI is also not available on this system.

No email check could be completed — please re-authenticate with Zapier MCP to restore email monitoring.
