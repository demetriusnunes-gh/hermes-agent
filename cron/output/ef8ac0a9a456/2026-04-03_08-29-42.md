...
No relevant emails in the last 2 hours. The 35 emails from the last day are all newsletters, promotions, or automated notifications that don't match any relevance criteria.

**State saved.** Zapier MCP token appears masked/truncated in both configs (`***` in config.yaml, partially truncated in mcporter.json), so IMAP was used as fallback for Gmail.
