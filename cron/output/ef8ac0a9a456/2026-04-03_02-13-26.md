...
⚠️ Email check skipped — Zapier MCP unreachable.

**Reason:** The configuration in `~/.hermes/config.yaml` contains a masked token (`***`) for the Zapier MCP server. This prevents authentication via `mcporter`, and the fallback browser-based OAuth timed out because this is a headless cron session.

**Action required:** Please update the `mcp_servers.zapier.url` value in your config with the current full token from your Zapier MCP dashboard.

```yaml
mcp_servers:
  zapier:
    url: "https://mcp.zapier.com/api/v1/connect?token=YOUR_FULL_TOKEN_HERE"
```
