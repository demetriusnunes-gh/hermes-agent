...
### 2. Fetch Recent Emails (Zapier MCP)
...
[Zapier MCP] OAuth issue for gmail_find_email (in:inbox newer_than:2h) and Google Calendar — timed out on the gateway side. This is a transient Zapier/API connectivity issue, not a missing email problem. Will retry on the next scheduled run.
